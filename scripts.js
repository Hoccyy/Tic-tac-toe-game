...
let playerChoice   = 'X';                                    // Initializing the player's choice as X for default

let pieceHolder = document.getElementById('piece-holder');   // Stores the container that holds each grid
...
pieceHolder.remove();                                        // Removes the pieces from grid temporarily for choice making

// Selection screen to choose your symbol
let choiceCanvas = document.createElement('div'); // Creates grid to make decision and styling for it
choiceCanvas.style = 'background-color: rgb(110, 91, 91);height: 50vh; width; 50px; border-radius: 13.33px; margin-right: 40px;\
...
optionO.style = 'color: darkred; font-size: 313px; font-weight: 700; height: 70%; width: 30vw; border-radius: 13px; margin: 4rem;\
background-color: white; box-shadow: 0 19px 38px rgba(0,0,0,0.30), 0 15px 12px rgba(0,0,0,0.22);';

optionX.style = 'color:darkblue; font-size: 313px; font-weight: 700; height: 70%; width: 30vw; border-radius: 13px; margin: 4rem;\
background-color: white; box-shadow: 0 19px 38px rgba(0,0,0,0.30), 0 15px 12px rgba(0,0,0,0.22);';
...
// Setting the choices for each button
optionO.innerHTML = 'O';                    optionX.innerHTML = 'X';

// Adds each choice button to the decision making screen
choiceCanvas.appendChild(optionO);          choiceCanvas.appendChild(optionX);


// Setting up GamePlay for when the player chooses 'X' or 'O'
...
// Runs the corresponding functions for each of the player's choices
optionX.onclick = playerChoiceX;            optionO.onclick = playerChoiceO;
...
let c=0; // Variable for grid traversal in favour of the player and then storing it as a value to check for the CPU's position
// Slot fill function inclusive of CPU moves
function selectedTile(tile, pieceName) {
    if (tile.querySelector('h1').innerHTML != '-') {
        return;
    ...
    // Clearing of the canvas to restart
    if (gameOver) {
        pieceHolder = document.getElementById('piece-holder');
        pieceHolder.remove();
        playingScreen.appendChild(choiceCanvas);// Asks the player their choice
        // Resetting variables and game
        gameOver = false;
        slotsFilled = 0;
    }
}